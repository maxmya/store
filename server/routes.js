...
const routes = {
  getCustomer: async (req, res) => {
    ...
    try {
      ...
      const query = 'SELECT customers.id,customers.given_name customers.role FROM customers WHERE customers.id = $1;';
      ...
      res.status(200).json({ name: customer.rows[0].given_name, cart: cartCount.total || 0,role:customer.role });
    } catch (err) {
      res.status(200).json({ name: null, cart: null });
    }
  },
