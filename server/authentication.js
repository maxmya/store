...
module.exports = () => {
  ...
  async function (accessToken, refreshToken, profile, cb) {
    ...
    // create new user
    query = `INSERT INTO customers(facebook_id, given_name, email, fullname, local) VALUES ($1, $2, $3, $4, $5) RETURNING *;`;
  ...
  async function (req, email, password, cb) {
    ...
    bcrypt.compare(password, hash, async function (error, res) {
      if (res) {
        query = ` SELECT customers.id, customers.given_name , customers.role FROM customers
        ...
        return cb(null, { name, cart: cartCount.total || 0, role: userRole, token: createToken(tokenObj)});
      }
